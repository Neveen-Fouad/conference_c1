...
class InterestsController extends Controller
{
    ...
    public function index()
    {
        $interests = Cache::remember('interests.all',now()->addHour(),function () { 
            return Interests::select('id', 'slug', 'name', 'description')->get();
        });
    ...
    public function clientInterests(Request $request)
{
    ...
    $client->interests()->syncWithoutDetaching($request->interests);
        ...
        $interests = Cache::remember("client_{$client->id}_interests",now()->addHour(),function () use ($client) {return $client->interests()->select('interests.id', 'slug', 'name')->get();});
